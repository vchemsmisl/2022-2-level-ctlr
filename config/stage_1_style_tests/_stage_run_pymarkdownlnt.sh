...
echo "Running pymarkdownlnt for README.md"
   python -m pymarkdown --config config/stage_1_style_tests/pymarkdownlnt.json scan **/*.md

if [[ $? -ne 0 ]]; then
  echo "pymarkdownlnt check failed for ${filename}."
  FAILED=1
else
  echo "pymarkdownlnt check passed for ${filename}."
fi
