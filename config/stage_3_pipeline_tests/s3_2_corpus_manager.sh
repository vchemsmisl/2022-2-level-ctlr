...
if [[ ${TARGET_SCORE} == 4 ]]; then
  ...
  python -m pytest -m "mark4 and stage_3_2_corpus_manager_checks" --capture=no
elif [[ ${TARGET_SCORE} == 6 ]]; then
  echo "Running score six checks"
  python -m pytest -m "mark6 and stage_3_2_corpus_manager_checks" --capture=no
elif [[ ${TARGET_SCORE} == 8 ]]; then
  echo "Running score eight checks"
  python -m pytest -m "mark8 and stage_3_2_corpus_manager_checks" --capture=no
  echo "TBD: later"
else
