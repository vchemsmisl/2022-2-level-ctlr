...
[Clone a fork of the repository to work locally](#cloning-fork) step and click `Open`
    ...
    * **NB**: You need to select the folder with the fork called `202X-2-level-ctlr`, not 
   the folder with a specific lab.


3. Click `OK`

    * **NB**: If in the `Base Interpreter` field the Python version is < 3.10, then 
...
3. Run `git commit -m "message"`
    ...
    * **NB**: It is recommended to use a brief description of the changes you made as `message`. 
